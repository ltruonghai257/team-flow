...
test.describe('Phase 18: Status Transition Graph', () => {
	...
	test('Status manager route is shareable via URL params', async ({ page }) => {
		...
		await expect(page.getByRole('button', { name: 'Transition rules' })).toBeVisible();
	...
	test('Transition rules tab shows matrix with draft and save states', async ({ page }) => {
		...
		if (!isVisible) {
			test.skip(true, 'User is not supervisor — cannot access status manager');
			return;
		}
	...
	test('Generate linear flow button creates draft transitions', async ({ page }) => {
		await page.goto('/tasks');
		await page.waitForLoadState('networkidle');
		...
		const matrixCheckboxes = page.locator('table tbody input[type="checkbox"]');
		...
		// Draft changes banner should appear
		await expect(page.getByText('Draft changes')).toBeVisible({ timeout: 5000 });
	...
	test('Kanban view loads and shows status columns', async ({ page }) => {
		...
		await expect(page.locator('[style*="touch-action"]').first()).toBeVisible();
	...
	test('Task edit modal shows status options', async ({ page }) => {
		...
		const editBtn = page.locator('.card button').filter({ has: page.locator('svg') }).first();
	...
	test('Status tab can be switched and persists in URL', async ({ page }) => {
		...
		const statusesTab = page.getByRole('button', { name: 'Statuses', exact: true });
		await expect(statusesTab).toHaveClass(/bg-indigo-600/);

		// Switch to Transition rules
		await page.getByRole('button', { name: 'Transition rules' }).click();
		await expect(page.getByRole('button', { name: 'Transition rules' })).toHaveClass(/bg-indigo-600/);
