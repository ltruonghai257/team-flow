...
- **Global scope**: `C:/Project/team-flow/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Project/team-flow/.claude/notes/`)
...
2. Glob `C:/Project/team-flow/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Project/team-flow/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Project/team-flow/.claude/notes/` — works in any directory
